...
WebAudioParamDescriptor testParam{
    .defaultValue = 0.f, .minValue = 0.f, .maxValue = 1.f, .automationRate = WEBAUDIO_PARAM_K_RATE};
...
bool ProcessAudio(int numInputs, const AudioSampleFrame *inputs, int numOutputs, AudioSampleFrame *outputs,
...
EM_JS(int, getSampleRate, (), { return new AudioContext().sampleRate; });
...
void PrepareAudio()
{
    xynth::ProcessSpec spec{
        .sampleRate = getSampleRate(), .numChannels = getNumberOfChannels(), .maxBlockSize = getMaximumBlockSize()};
...
void AudioWorkletProcessorCreated(EMSCRIPTEN_WEBAUDIO_T audioContext, bool success, void *userData)
{
    ...
    EmscriptenAudioWorkletNodeCreateOptions options = {
        .numberOfInputs = 0, .numberOfOutputs = 1, .outputChannelCounts = outputChannelCounts};

    // Instantiate the noise-generator Audio Worklet Processor.
    EMSCRIPTEN_AUDIO_WORKLET_NODE_T wasmAudioWorklet =
        emscripten_create_wasm_audio_worklet_node(audioContext, "audio-processor", &options, &ProcessAudio, userData);
...
void WebAudioWorkletThreadInitialized(EMSCRIPTEN_WEBAUDIO_T audioContext, bool success, void *userData)
{
    ...
    WebAudioWorkletProcessorCreateOptions opts = {.name = "audio-processor",
...
int main()
{
    ...
    std::cout << "Browser Sample Rate: " << getSampleRate() << ", numChannels: " << getNumberOfChannels()
    ...
    emscripten_start_wasm_audio_worklet_thread_async(context, wasmAudioWorkletStack, sizeof(wasmAudioWorkletStack),
                                                     WebAudioWorkletThreadInitialized, &userData);
}
